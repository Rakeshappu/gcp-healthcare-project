...
CREATE TABLE IF NOT EXISTS `healthcare-497406.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `healthcare-497406.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `healthcare-497406.silver_dataset.claims` c
